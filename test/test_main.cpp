...
//--------------
//
vector<string> LoadFileLines(string const &filename)
{
  string line;
  vector<string> lines;
...
string LoadFile(string const &filename)
{
  ...
  if (in)
  {
    ...
    line.resize(
        (size_t)in.tellg()); //- Cast is there to make 32-bit Windows happy
...
string MakeFilePath(std::string const &dir, std::string const &filename)
{
  string path;
